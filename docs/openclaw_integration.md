...
- **Standard Header / Session Key**:
  *   **Standard Key Format**: `agent:<agentId>:domain-expansion-ar-game:<sessionId>`
  *   **Telegram & Character Dashboard Integration Format**: When `OPENCLAW_SESSION_ID` is set to a Telegram session identifier (e.g. `telegram:228465334`), we dynamically align the session formats:
      *   **AgentCore Runtime SHA-1 Compliant Session ID**: To fit character dashboard restrictions and Bedrock requirements, the session is formatted as `dashboard_session_<sha1_hash>` where `<sha1_hash>` is the first 24 characters of the SHA-1 hash of the Telegram string (e.g. `695b2cc9a0a34e2b738c8c35`).
      *   **OpenClaw session key mapping**: For OpenClaw-compatible routes, the header is passed as `x-openclaw-session-key: agent:<agentId>:telegram` to route conversation memory back to the user's specific chat channel on the live dashboard.
  
  這告訴 OpenClaw 繞過動態 UUID 產生，並在專屬遊戲標籤下（`domain-expansion-ar-game`）或 Telegram 整合頻道下載入持久會話資料庫。這允許用戶輕鬆地查詢、管理或批次刪除該遊戲產生的所有會話。
  
- **User Param & Actor Mappings**:
  *   `user: sessionId` (or the parsed user ID from the Telegram context)
  *   `actorId: sessionId` (to specify the caller channel identity)
  
  在 OpenAI 本文負載或 OpenClaw 自定義承載中指定唯一的遊戲會話 ID 及 Actor 識別，以符合一等（first-class）的 OpenAI 結構定義與動態儀表板追蹤。
